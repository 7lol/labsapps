...
public class CsvDataTest {
    ...
    prepere("JACEK","PRO","WROCLAW","POLAND");
        if (!data.getData().contains("POLAND") || !data.getData().contains("WROCLAW") || !data.getData().contains("JACEK") || !data.getData().contains("PRO"))
    ...
    public void shouldHaveSameNumberOfDataAsHeaderHas() throws Exception {
        ...
        if (data.getData().split(";").length!= CsvData.getHeader().split(";").length)
            throw new AssertionError();
    }
